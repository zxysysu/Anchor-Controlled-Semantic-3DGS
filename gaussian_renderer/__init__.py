...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, 
           scaling_modifier=1.0, visible_mask=None, retain_grad=False, render_semantic=True):
    """
    Render the scene. 
    ...
    # === 修改：使用新的光栅化器，同时渲染颜色和语义 ===
    if render_semantic:
        # 同时渲染颜色和语义特征
        rendered_image, rendered_semantic, radii, depth = rasterizer(
            means3D=xyz,
            means2D=screenspace_points,
            shs=None,
            colors_precomp=color,
            semantic_feature=semantic,  # 传入语义特征
            opacities=opacity,
            scales=scaling,
            rotations=rot,
            cov3D_precomp=None
        )
        # print(rendered_semantic)
    ...
    if is_training:
        ...
        if render_semantic:
            result.update({
                "semantic_logits": rendered_semantic,  # 语义概率图 [C, H, W]
                "semantic_color": semantic_color,      # 彩色语义图 [3, H, W]
                "depth": depth                         # 深度图 [1, H, W]
            })
        return result
    else:
        result = {
        ...
        if render_semantic:
            result.update({
                "semantic_logits": rendered_semantic,  # 语义概率图
