...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # 添加语义图片路径
        seg_folder = os.path.join(os.path.dirname(images_folder), "segment_pred")
        # 获取不带扩展名的文件名
        base_name = os.path.splitext(os.path.basename(extr.name))[0]
        semantic_image_path = os.path.join(seg_folder, base_name + '_color.png')
        
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, 
                              semantic_image_path=semantic_image_path,  # 添加语义路径
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", is_debug=False, undistorted=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            seg_folder = os.path.join(path, "segment_pred")
            semantic_image_path = os.path.join(seg_folder, os.path.basename(frame["file_path"] + extension))
