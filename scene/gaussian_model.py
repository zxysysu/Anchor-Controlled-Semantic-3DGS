...
class GaussianModel:
    ...
    def __init__(self, 
                 ...
                 num_semantic_classes: int = 9  # 添加语义类别数
                 ):
        ...
        self.num_semantic_classes = num_semantic_classes  # 保存语义类别数
        ...
        #     nn.ReLU(True),
        #     nn.Linear(feat_dim, self.num_semantic_classes),  # 每个锚点一个语义特征
        # ).cuda()
        self.mlp_semantic = nn.Sequential(
            nn.Linear(feat_dim + 3 + self.appearance_dim, feat_dim*2),
            nn.BatchNorm1d(feat_dim*2),  # 添加批量归一化
            nn.ReLU(True),
            nn.Linear(feat_dim*2, feat_dim),
            nn.BatchNorm1d(feat_dim),  # 添加批量归一化
            nn.ReLU(True),
            nn.Linear(feat_dim, self.num_semantic_classes),
        ).cuda()
    ...
    def eval(self):
        ...
        self.mlp_cov.eval()
        self.mlp_color.eval()
        self.mlp_semantic.eval()  # 语义MLP评估模式
    ...
    def train(self):
        ...
        self.mlp_semantic.train()  # 语义MLP训练模式
        if self.appearance_dim > 0:
    ...
    @property
    def get_semantic_mlp(self):  # 获取语义MLP
    ...
    def training_setup(self, training_args):
        ...
        if self.use_feat_bank:
            l = [
                ...
                {'params': self.mlp_semantic.parameters(), 'lr': training_args.mlp_semantic_lr_init, "name": "mlp_semantic"},  # 语义MLP
    ...
    def update_learning_rate(self, iteration):
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_color":
                ...
                param_group['lr'] = lr
            if param_group["name"] == "mlp_semantic":  # 语义MLP学习率更新
    ...
    def save_mlp_checkpoints(self, path, mode = 'split'):
        ...
        if mode == 'split':
            ...
            semantic_mlp = torch.jit.trace(self.mlp_semantic, (torch.rand(1, self.feat_dim+3+self.appearance_dim).cuda()))
            ...
            state_dict = {
                ...
                'semantic_mlp': self.mlp_semantic.state_dict(),  # 添加语义MLP
            }
            if self.use_feat_bank:
    ...
    def load_mlp_checkpoints(self, path, mode = 'split'):
        if mode == 'split':
            ...
            self.mlp_semantic = torch.jit.load(os.path.join(path, 'semantic_mlp.pt')).cuda()  # 加载语义MLP
            if self.use_feat_bank:
            ...
            self.mlp_semantic.load_state_dict(checkpoint['semantic_mlp'])  # 加载语义MLP
            if self.use_feat_bank:
